...
if($dataTahun and $dataBulan) {
	if($dataBulan == "00") {
		...
		$filterSQL = "WHERE LEFT(tgl_pembelian,4)='$dataTahun' AND MID(tgl_pembelian,6,2)='$dataBulan'";
	}
}
else {
	$filterSQL = "";
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self">
  <table width="450" border="0"  class="table-list">
    ...
    <tr>
      <td width="119"><strong>Bulan Pembelian </strong></td>
  ...
  <tr>
	...
	<table class="table-list" width="100%" border="0" cellspacing="1" cellpadding="2">
      <tr>
        ...
        <th width="224"><strong>Supplier </strong></th>
        <th width="126"><strong>Tgl. Pembelian </strong></th>
        <th width="127"><strong>No. Pembelian </strong></th>
	...
	$mySql = "SELECT pembelian.*, supplier.nm_supplier FROM pembelian 
			 ...
			 $filterSQL
			 ORDER BY pembelian.no_pembelian DESC LIMIT $halaman, $baris";
	...
	while ($myData = mysql_fetch_array($myQry)) {
		...
		$my2Sql = " SELECT SUM(harga_beli * jumlah) As total_belanja FROM pembelian_item WHERE no_pembelian = '$Kode'";
