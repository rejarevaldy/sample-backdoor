...
if(isset($_GET['Aksi'])){
	...
	if(trim($Aksi)=="Delete"){
		...
		$mySql  = "DELETE FROM tmp_returjual WHERE id='$id' AND kd_user='$kodeUser'";
...
if(isset($_POST['btnTambah'])){
	...
	if (trim($cmbPelanggan)=="Kosong") {
		$pesanError[] = "Data <b>Pelanggan</b> belum diisi, pilih pada combo !";		
	}
	if (trim($txtBarcode)=="") {
	...
	if (count($pesanError)>=1 ){
		...
		# SIMPAN KE DATABASE (tmp_returjual)	
		...
		if($myQty >= 1) {
			...
			$cekSql ="SELECT * FROM tmp_returjual WHERE kd_barang='$kodeBarang' AND kd_user='$kodeUser'"; 
			$cekQry = mysql_query($cekSql, $koneksidb) or die ("Gagal Query".mysql_error());
			if(mysql_num_rows($cekQry) >= 1) {
				// Jika tadi sudah dipilih, cukup jumlahnya diupdate
				$tmpSql = "UPDATE tmp_returjual SET jumlah = jumlah + $txtJumlah WHERE kd_barang='$kodeBarang' AND kd_user='$kodeUser'"; 
				...
				$tmpSql 	= "INSERT INTO tmp_returjual (kd_pelanggan, kd_barang,jumlah,  keterangan, kd_user) 
					VALUES ('$cmbPelanggan', '$kodeBarang', '$txtJumlah', '$txtKeteranganBrg',  '$kodeUser')";
...
if(isset($_POST['btnSimpan'])){
	...
	if (trim($cmbPelanggan)=="") {
		$pesanError[] = "Data <b>Pelanggan</b> belum diisi, pilih pada combo !";		
	}
	
	# Periksa apakah sudah ada barang yang dimasukkan
	$tmpSql ="SELECT COUNT(*) As qty FROM tmp_returjual WHERE kd_user='$kodeUser'";
	...
	if (count($pesanError)>=1 ){
		...
		# Jika jumlah error pesanError tidak ada, maka penyimpanan dilakukan. Data dari tmp dipindah ke tabel returjual dan returjual_item
		$noTransaksi = buatKode("returjual", "RB");
		$mySql	= "INSERT INTO returjual (no_returjual, tgl_returjual, kd_pelanggan, keterangan, kd_user) 
						VALUES ('$noTransaksi', '$txtTanggal', '$cmbPelanggan', '$txtKeterangan', '$kodeUser')";
		...
		$tmpSql ="SELECT * FROM tmp_returjual WHERE kd_user='$kodeUser'";
		$tmpQry = mysql_query($tmpSql, $koneksidb) or die ("Gagal Query Tmp".mysql_error());
		while ($tmpData = mysql_fetch_array($tmpQry)) {
			...
			$itemSql = "INSERT INTO returjual_item SET 
									no_returjual='$noTransaksi', 
									kd_barang='$dataKode', 
									jumlah='$dataJumlah', 
		...
		$hapusSql = "DELETE FROM tmp_returjual WHERE kd_user='$kodeUser'";
		mysql_query($hapusSql, $koneksidb) or die ("Gagal kosongkan tmp".mysql_error());
		
		// Refresh form
		echo "<meta http-equiv='refresh' content='0; url=?open=Returjual-Baru'>";
	}	
}
...
$dataPelanggan	= isset($_POST['cmbPelanggan']) ? $_POST['cmbPelanggan'] : '';
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self">
  <table width="800" cellpadding="3" cellspacing="1"  class="table-list">
    <tr>
      <td colspan="3"><h1>RETUR PENJUALAN BARANG</h1></td>
    </tr>
    <tr>
    ...
    <tr>
      <td width="26%"><strong>No. Retur </strong></td>
    ...
    <tr>
      ...
      <a href="javaScript: void(0)" onclick="popup('pencarian_pelanggan.php')" target="_self"><b> Cari Pelanggan </b></a></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><strong>:</strong></td>
      <td><input name="txtNama" value="<?php echo $dataNamaBrg; ?>" type="text" id="txtNama" size="80" maxlength="200" readonly="readonly" /></td>
    </tr>
    <tr>
  ...
  <table class="table-list" width="800" border="0" cellspacing="1" cellpadding="2">
    ...
    <tr>
      ...
      <td width="199" bgcolor="#CCCCCC"><strong>Keterangan Barang </strong></td>
      <td width="48" align="right" bgcolor="#CCCCCC"><strong>Jumlah</strong></td>
      <td width="39" align="center" bgcolor="#CCCCCC">&nbsp;</td>
    </tr>
<?php
// Qury menampilkan data dalam Grid TMP_returjual 
$tmpSql ="SELECT barang.nm_barang, tmp.* FROM tmp_returjual As tmp
...
$myQryStok 	= mysql_query($mySql, $koneksidb)  or die ("Query ambil data salah : ".mysql_error());
$myDataStok	= mysql_fetch_array($myQryStok);
