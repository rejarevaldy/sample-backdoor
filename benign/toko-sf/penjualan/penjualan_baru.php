...
if(isset($_POST['btnTambah'])){
	...
	if (count($pesanError)>=1 ){
		...
		if($myQty >= 1) {
			...
			$cekSql ="SELECT * FROM tmp_penjualan WHERE kd_barang='$kodeBarang' AND kd_user='$kodeUser' AND kd_pelanggan='$cmbPelanggan'"; 
			...
			if(mysql_num_rows($cekQry) >= 1) {
				...
				$tmpSql 	= "INSERT INTO tmp_penjualan (kd_pelanggan, kd_barang, ppn,harga, jumlah,  kd_user) 
								VALUES ('$cmbPelanggan', '$kodeBarang', $txtPPN, '$txtHarga', '$txtJumlah',  '$kodeUser')";
...
if(isset($_POST['btnSimpan'])){
	...
	if (count($pesanError)>=1 ){
		...
		$mySql	= "INSERT INTO penjualan (no_penjualan, tgl_penjualan, keterangan, kd_pelanggan, 
						...
						VALUES ('$noTransaksi', '$txtTglTransaksi', '$txtKeterangan', '$cmbPelanggan', 
		...
		while ($tmpData = mysql_fetch_array($tmpQry)) {
			...
			$itemSql = "INSERT INTO penjualan_item (no_penjualan, kd_barang, ppn, harga_modal, harga_jual, jumlah) 
						VALUES ('$noTransaksi', '$dataKode',  $ppn, '$dataHrgModal', '$dataHrgJual', '$dataJumlah') ";
...
$dataPelanggan		= isset($_POST['cmbPelanggan']) ? $_POST['cmbPelanggan'] : $kodePelanggan;
...
if(mysql_num_rows($myQry)>=1)  {
	$dataLevel		= $myData['level_harga'];
	echo "Level: ".$myData['level_harga'];
}
else {
	$dataLevel	= "4";
}
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self">
  <table width="900" cellpadding="3" cellspacing="1"  class="table-list">
    ...
    <tr>
      ...
      <td><strong>:</strong></td>
      <td><select name="cmbPelanggan" id="idKodePlg" onchange="javascript:submitform();">
...
?>
    <tr>
      ...
      <td><a href="?Aksi=Delete&id=<?php echo $id; ?>&kodePelanggan=<?php echo $dataPelanggan; ?>&Level=<?php echo $dataLevel; ?>" target="_self">Delete</a></td>
    </tr>
<?php } ?>
    ...
    <tr>
      <td colspan="7" align="right"><input name="btnSimpan" type="submit" style="cursor:pointer;" value=" SIMPAN TRANSAKSI " /></td>
      <td>&nbsp;</td>
    </tr>
  </table>
</form>
