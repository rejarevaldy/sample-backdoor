...
if(isset($_POST['btnCetak'])) {
	// Buka file
	echo "<script>";
	echo "window.open('cetak/barang_maksimal.php?kodeKategori=$dataKategori', width=330)";
	echo "</script>";
}
...
$pageSql 	= "SELECT * FROM barang LEFT JOIN jenis ON barang.kd_jenis=jenis.kd_jenis WHERE stok >= stok_maksimal  $filterSQL";
$pageQry 	= mysql_query($pageSql, $koneksidb) or die("Error paging:".mysql_error());
$jmlData	= mysql_num_rows($pageQry);
$maksData	= ceil($jmlData/$baris);
?>
<h2> LAPORAN DATA BARANG STOK MAKSIMAL/ AMAN </h2>
...
<table class="table-list" width="958" border="0" cellspacing="1" cellpadding="2">
  ...
  <?php
	...
	$mySql 	= "SELECT barang.*, jenis.nm_jenis FROM barang LEFT JOIN jenis ON barang.kd_jenis=jenis.kd_jenis 
				WHERE barang.stok >= stok_maksimal 
				$filterSQL 
  ...
  <tr class="selKecil">
	...
	for ($h = 1; $h <= $maksData; $h++) {
		...
		echo " <a href='?open=Laporan-Barang-Stok-Maksimal&hal=$list[$h]&kodeKat=$dataKategori'>$h</a> ";
	}
	?></td>
  </tr>
</table>
<a href="cetak/barang_maksimal.php?kodeKategori=<?php echo $dataKategori; ?>" target="_blank"><img src="images/btn_print2.png" height="18" border="0" title="Cetak ke Format HTML"/></a>
<?php 
// Penutup Hak Akses
}
else {
